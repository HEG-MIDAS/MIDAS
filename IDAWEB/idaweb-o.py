                ...
                if measures != "":
                    ...
                    if measures[0] != "time":
                        try:
                            ...
                            timestamp = o_timestamp + datetime.timedelta(hours=0)
                            timestamp = timestamp.strftime('%Y-%m-%d %H:%M:%S')
                            if timestamp not in dataset[station_abbr[station_name]]:
                                dataset[station_abbr[station_name]][timestamp] = copy.deepcopy(header_station_file)
